...
- **Network connectivity** blade
  ...
  - **Azure Private DNS zone** - Select 'Create new zones' for the automation to deploy new private DNS zones for Azure files and keyvault or select 'Use existing zones' to provide existing private DNS zone for Azure File share and Key vault (Private DNS for Azure files is required for FSLogix configuration to complete).
